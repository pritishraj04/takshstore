...
export const RequirePermission = (moduleName: string, level: 'READ' | 'WRITE') => 
...
export class AdminPermissionsGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    let user;
    try {
      user = this.jwtService.verify(token, { secret: process.env.ADMIN_JWT_SECRET || 'tmp_dev_secret_change_me_in_prod' });
    } catch (e) {
    ...
    const isSuperOnly = this.reflector.get<boolean>('admin_super_only', context.getHandler());
    if (isSuperOnly && !user.isSuper) {
      throw new ForbiddenException('Super Admin access required');
    }

    // Check specific module permissions
    const requiredPermission = this.reflector.get<{ moduleName: string; level: 'READ' | 'WRITE' }>(
      'admin_permission',
      context.getHandler()
    );
    ...
    if (requiredPermission) {
      ...
      if (userLevel === 'NONE') {
        throw new ForbiddenException(`Missing permission for module: ${moduleName}`);
      }

      if (level === 'WRITE' && userLevel !== 'WRITE') {
        throw new ForbiddenException(`WRITE permission required for module: ${moduleName}`);
      }
    }
