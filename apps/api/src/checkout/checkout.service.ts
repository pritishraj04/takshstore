...
export class CheckoutService {
    ...
    async initiateCheckout(userId: string, totalAmount: number, items: any[], shippingAddress?: any, developerNotes?: string) {
        ...
        for (const item of items) {
           ...
           calculatedTotal += (product.discountedPrice || product.price) * item.quantity;
        }
                ...
                items: {
                    create: items.map((item) => {
                        const baseItem: any = {
                            ...
                            priceAtPurchase: item.priceAtPurchase, // Still keeping record of what frontend saw
                        };
        ...
        // Initialize Razorpay
        const rzpOrder = await this.razorpayService.createOrder(amountToPay, order.id);

        if (!rzpOrder || !rzpOrder.id) {
            throw new InternalServerErrorException('Failed to secure order with Razorpay');
        }
    ...
    async verifyCheckout(razorpayPaymentId: string, razorpayOrderId: string, razorpaySignature: string) {
