...
export class DigitalInvitesController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Get all digital invites for the authenticated user',
  })
  ...
  @Get('check-slug/:slug')
  @ApiOperation({
    summary: 'Check if a custom URL slug is available for a digital invite',
  })
  ...
  @ApiOperation({
    summary:
      'Get a draft digital invite by its database ID for the Customizer editor',
  })
  ...
  @Get(':slug')
  @ApiOperation({
    summary: 'Get a digital invite by its custom slug for public viewing',
  })
  @ApiResponse({ status: 200, description: 'Return the digital invite.' })
  ...
  async updateInvite(
    @Param('id') id: string,
    @Body() body: { inviteData: any; status?: any },
    @Request() req,
  ) {
    const userId = req.user?.sub || req.user?.userId;
    return this.digitalInvitesService.updateInvite(
      id,
      userId,
      body.inviteData,
      body.status,
