...
export class DigitalInvitesService {
  ...
  async updateInvite(
    ...
    if (isPaid) {
      ...
      const incomingData = inviteData;

      // Guard A: Typo-Tolerance (Max 3 character changes allowed)
      const existingBrideName = (
        existingInvite.originalBrideName || ''
      ).toLowerCase();
      const incomingBrideName = (
        incomingData?.couple?.bride?.name || ''
      ).toLowerCase();
      const existingGroomName = (
        existingInvite.originalGroomName || ''
      ).toLowerCase();
      const incomingGroomName = (
        incomingData?.couple?.groom?.name || ''
      ).toLowerCase();

      const brideDistance = getEditDistance(
        existingBrideName,
        incomingBrideName,
      );
      const groomDistance = getEditDistance(
        existingGroomName,
        incomingGroomName,
      );
      ...
      if (brideDistance > 3 || groomDistance > 3) {
        throw new BadRequestException(
          'Major name changes are locked. You can only correct minor typos (max 3 characters) from the original purchased names.',
        );
      }

      // Guard B: Date Horizon Lock (Max 90 days from original)
      if (
        existingInvite.originalEventDate &&
        incomingData?.celebrations &&
        Array.isArray(incomingData.celebrations)
      ) {
        ...
        if (times.length > 0) {
          ...
          if (incomingLatestDate > horizonDate) {
            throw new BadRequestException(
              'Event dates cannot be postponed more than 90 days from the original purchased date.',
            );
          }
        }
      }

      // Guard C: Eternity Time-Lock Lock
      if (existingInvite.marriageDate) {
        const now = new Date();
        const marriageDate = new Date(existingInvite.marriageDate);
        if (now > marriageDate && !existingInvite.isEternity) {
          throw new ForbiddenException(
            'This invite has expired and can no longer be edited',
          );
        }
      }
    }
    ...
    const normalizedSlug = payloadSlug
      ? payloadSlug.toLowerCase().replace(/[^a-z0-9-]/g, '')
      : null;
    ...
    return this.prisma.$transaction(async (tx) => {
      const updatedInvite = await tx.digitalInvite.update({
        where: { id },
        data,
  ...
  async deleteDraft(userId: string, id: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (
        invite.orderItem.order.status === 'PENDING' &&
        invite.orderItem.order.totalAmount === 0
      ) {
